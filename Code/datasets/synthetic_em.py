...
class SyntheticReal(SyntheticData):
    '''
...
class SyntheticFourier(SyntheticData):
    '''
...
def synthetic_base(data_all, synthetic_dataset, batch, norm=False, shuffle=True):
    '''
    Base function for getting synthetic cryo-EM dataset.
...
def us_synthetic_real(path, batch, fc, fn, picked=None, norm=False):
    '''
    Get synthetic cryo-EM dataset in spatial domain.
    ...
    train_loader = synthetic_base(data_all, SyntheticReal, batch, norm=norm)
    return train_loader


def us_synthetic_fourier(path, batch, fc, fn, picked=None, norm=False):
    '''
    Get synthetic cryo-EM dataset in Fourier domain.
    ...
    if picked is None:
        picked = [0]
    data_all = get_all(path, lambda a, x, y: [a], fc=fc, fn=fn, picked=picked)
    train_loader = synthetic_base(data_all, SyntheticFourier, batch, norm=norm)
    return train_loader
