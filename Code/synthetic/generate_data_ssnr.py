...
def change_f(img, pixel_size):
    '''
    Changing SSNR at low frequencies. low frequency radius: 10.

    Args:
        img: input image in ndarray

    Returns:
        image with SSNR changed at low frequencies
    '''
    f = np.fft.fftshift(np.fft.fft2(img))
    l = int(10 / pixel_size) # radius of 10 or 20, please modify
    shape = (2 * l, 2 * l)
    delta = np.mean(np.abs(f) * 3)
    g_r = np.random.normal(0, delta, shape)
    g_i = np.random.normal(0, delta, shape)
    g = g_r + g_i * 1j
    cx = int(f.shape[0] / 2)
    cy = int(f.shape[1] / 2)

    for x in range(cx - int(shape[0] / 2), cx + int(shape[0] / 2)):
        for y in range(cy - int(shape[1] / 2), cy + int(shape[1] / 2)):
            if (x - cx)**2 + (y - cy)**2 <= l**2:
                f[x, y] += g[x - cx + int(shape[0] / 2), y - cy + int(shape[1] / 2)]

    c = np.fft.ifft2(np.fft.ifftshift(f)).real
    return c
...
def generate_data_all(path_input, index, pixel_size_g):
    '''
    ...
    path = path_input + str(index) + '/'
    img_ori = cv2.imread(path + 'ori.png')
    img_ori = cv2.cvtColor(img_ori, cv2.COLOR_RGB2GRAY)

    pixel_size = pixel_size_g[index - 1]
    if img_ori.shape[0] != 128:
        pixel_size = 128 / img_ori.shape[0] * pixel_size
...
if __name__ == '__main__':
    ...
    path_input = '/path/to/synthetic data'  # folder 'cryo-em centers' in Supplementary File

    pixel_size_g = [2.08, 2, 1.23, 1.42, 2.8, 1.461, 1.22]

    pool = multiprocessing.Pool(6)
    for i in range(7):
        pool.apply_async(generate_data_all, args=(
            path_input,
            i + 1,
            pixel_size_g,
        ))
