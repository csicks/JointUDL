...
from datasets import real_fourier
...
class AutoEncoder(nn.Module):
    '''
    Class for the autoencoder in the joint alignment network.
    '''
    ...
    def forward(self, x):
        ...
        Args:
            x: input image batch of size batch*c*128*128

        Returns:
            reconstructed image batch of size batch*c*128*128
        '''
        r = self.encoder(x)
        r = self.decoder(r)
        return r
...
class Net(nn.Module):
    '''
    Class for the pair-wise alignment architecture.
    '''
...
def transform(data, t):
    '''
    Function to transform images in one batch according to the results of pair-wise image alignment network.

    Args:
        data: one batch of images of size batch*2*128*128
        target: rotation angle to one batch of images of size batch*1

    Returns:
        transformed images of size batch*2*128*128
    '''
...
def train(epoch, model, ae, train_loader, device, optimizer_model, optimizer_ae, logger):
    '''
    Function to train the network.

    Args:
        epoch: epoch number
        model: pair-wise alignment network
        ae: autoencoder
        train_loader: dataloader for training
        device: cpu or gpu
        optimizer_model: optimizer for pair-wise alignment network
    ...
    for batch_idx, (data, image) in enumerate(train_loader):
        ...
        loss_p = angle_loss(mid_m, dm) + loss_r0 + loss_r1

        img_t = transform(data, output0)
        img_r = ae(img_t)

        loss_ae = F.mse_loss(img_r, img_t) * epoch * 20
        loss = loss_ae + loss_p
...
def main_synthetic(logger, name, continue_flag=False):
    '''
    Main function. Note in cryo-EM single particle images, we aim to align this dataset accurately without groundtruth instead of training a generalizable model (which may also be possible). Therefore, only training function is given. For real-world cryo-EM dataset, it might be hard to train from the very begining. Therefore, training on synthetic datasets generated from its cluster centers might be a wise choice to initialize the network.
    ...
    batch = 32
    path = '/path/to/real-world cryo-EM dataset/'
    train_loader = real_fourier(path, batch)
    ...
    if continue_flag:
        checkpoint = torch.load(checkpoint_path) # Continue training
        ...
        current_epoch = 0
        checkpoint = torch.load('/path/to/initialization')  # Training from initialization
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer_model.load_state_dict(checkpoint['optimizer_state_dict'])
        current_epoch = checkpoint['epoch']
        init_kaiming_normal(ae)

    for epoch in range(current_epoch + 1, 1000 + 1):
        train(epoch, model, ae, train_loader, device, optimizer_model, optimizer_ae, logger)
        torch.save(
            {
                'epoch': epoch,
